...
### 1.10 ViewBuilder closure 内不能声明 func / struct / class ⚠️ 高频踩坑

**犯错频率**: Pace v0.4.10 RouteDetailView BigRouteShapes.

**症状**: `Closure containing a declaration cannot be used with result builder 'ViewBuilder'`

**触发**: `GeometryReader { geo in ... }` / `VStack { ... }` / `body: some View` 等
ViewBuilder 闭包内写了 `func` 嵌套声明.

```swift
// ❌ ViewBuilder closure 内声明 func
private struct BigRouteShapes: View {
    var body: some View {
        GeometryReader { geo in
            let scaleX: CGFloat = geo.size.width / 280
            // ★ 报错: Closure containing a declaration cannot be used with ViewBuilder
            func vp(_ x: CGFloat, _ y: CGFloat) -> CGPoint {
                CGPoint(x: x * scaleX, y: y * scaleY)
            }
            Path { p in p.move(to: vp(45, 215)) }
        }
    }
}

// ✅ 改成 let closure 绑定 (闭包是 value, 不是 declaration)
GeometryReader { geo in
    let scaleX: CGFloat = geo.size.width / 280
...
**根因**: `@resultBuilder` 闭包 body 只允许 expressions + `let/var` 绑定 + control flow,
不允许 nested `func` / `struct` / `class` / `enum` 等 declaration. 这是 Swift 5.4 已
有的硬约束 (跟 iOS 14 无关, 跟 result builder 设计有关).

**关键区分**: Shape 协议的 `path(in rect: CGRect) -> Path` body 是**普通函数体**, 可以
嵌套 func; 但 SwiftUI 的 `body: some View` / `GeometryReader { in }` / `VStack { }`
等 ViewBuilder closure body 不可以.

```swift
// ✅ Shape.path(in:) 普通函数体, 可以塞 func
struct MyShape: Shape {
    func path(in rect: CGRect) -> Path {
        let scaleX = rect.width / 280
        func vp(_ x: CGFloat, _ y: CGFloat) -> CGPoint { ... }   // ★ 这里 OK
        var p = Path()
        p.move(to: vp(45, 215))
        return p
    }
...
// ❌ View.body ViewBuilder closure, 不可以
struct MyView: View {
    var body: some View {
        GeometryReader { geo in
            func vp(...) -> CGPoint { ... }   // ★ 这里炸
...
**跟 §1.2 雷 3c 的关系**: 雷 3c 建议把闭包改成具名 func 来稳类型推断,
**那条规则只对 Shape `path(in:)` 普通函数体适用**, 不适用于 ViewBuilder closure.
ViewBuilder 内反过来 — 用 `let f: (T) -> U = { ... }` 绑定闭包.

**预防 checklist**:
- 写 GeometryReader / VStack 内的辅助函数: 优先 `let f: (CGFloat, CGFloat) -> CGPoint = { ... }`
- 写 Shape `path(in:)` 内的辅助函数: 可以 `func vp(...) -> CGPoint { ... }`
- 看到 "Closure containing a declaration cannot be used with result builder" 立刻
  把附近的 `func` 改 `let closure`

**犯错记录**: v0.4.10 RouteDetailView BigRouteShapes — GeometryReader closure 内
写 `func vp(_:_:) -> CGPoint`, Xcode 报红. 改成 `let vp: (CGFloat, CGFloat) -> CGPoint = { x, y in ... }` 通过.
...
| v0.4.10 | "Closure containing a declaration..." | ViewBuilder closure 内嵌 func | §1.10 改 let closure |
| v0.4.10 | "Extra argument in call" | RecoveryDetailView Group 11 子 | §1.1 拆 2 Group |
